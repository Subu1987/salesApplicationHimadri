	...
	return BaseController.extend("com.infocus.salesApplication.controller.Home", {
		...
		validateInputs: function() {
			...
			var getInputIdsToValidate = function() {
				...
				if (isSingleCustomer) {
					return oSelectedIndex === 0 ? ["_customerInputId", "_materialInputId", "_financialYearInputId"] : ["_customerInputId",
						"_materialInputId", "_quarterInputId",
						"_quarterInputYearId"
					];
				} else {
					return oSelectedIndex === 0 ? ["_materialInputId", "_financialYearInputId"] : ["_materialInputId", "_quarterInputId",
						"_quarterInputYearId"
					];
				}
			};
		...
		onSearchMaterialMaster: function(oEvent) {
			var sQuery = oEvent.getParameter("newValue");
			var oList = Fragment.byId(this.getView().getId(), "idMaterialMasterList");
			if (!oList) return;
		...
		_resetMaterialMasterDialog: function() {
			var oList = Fragment.byId(this.getView().getId(), "idMaterialMasterList");
			var oSearchField = Fragment.byId(this.getView().getId(), "idMaterialSearchField");
		...
		_handleValueQuarterClose: function(oEvent) {
			...
			if (aSelectedItems && aSelectedItems.length > 0) {
				...
				if (oQuarterInput) {
					...
					oQuarterInput.setValueState("None"); // eset error state
		...
		onMaterialInputLiveChange: function(oEvent) {
			var oGlobalModel = this.getOwnerComponent().getModel("globalData");
			var sValue = oEvent.getParameter("value");
			if (!sValue) {
				oGlobalModel.setProperty("/selectedMaterialNames", []);
		...
		onRadioButtonSelectDomesticExport: function(oEvent) {
			...
			if (sSelectedKey === 0) { // Domestic 

				oGlobalDataModel.setProperty("/selectedDomesticExport", "D");

			} else if (sSelectedKey === 1) { // Export selected
		...
		_buildFilters: function(oGlobalData, oSelectedIndex) {
			...
			// Add Material Master to the filter
			if (aSelectedMaterialMasterData.length > 0) {
				filters.push(new Filter({
					filters: aSelectedMaterialMasterData.map(function(master) {
						return new Filter("materialGroup", FilterOperator.EQ, master);
					}),
					and: false
				}));
			}
			...
			if (aSelectedDomesticExport.length > 0) {
				filters.push(new Filter("domesticExport", FilterOperator.EQ, aSelectedDomesticExport));
			}
		...
		_onChartSelectFiscalYearWise: function(sFragmentId, oEvent) {
			...
			var oPopoverModel = new sap.ui.model.json.JSONModel({
				...
				Turnover: oDataContext["Turn Over (Cr)"],
